...
const Collections = async () => {
  ...
  // Mapping collection ids to their respective paths
  const collectionLinks: { [key: string]: string } = {
    "3": "/best_sellers",
    "2": "/new_arrivals",
    "1": "/prota",
  };
      ...
      {!collections || collections.length === 0 ? (
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 items-center justify-center">
          {collections.map((collection: CollectionAttributes) => {
            // Get the path for each collection using its id
            const collectionPath = collectionLinks[collection.id.toString()];

            return (
              <Link
                href={collectionPath || `/collections/${collection.id}`}
                key={collection.id}
              >
                <Image
                  key={collection.id}
