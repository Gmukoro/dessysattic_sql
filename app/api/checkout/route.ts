...
export async function POST(req: NextRequest) {
  try {
    const { cartItems, customer, delivery, currency } = await req.json();
    ...
    // Calculate total amount in cents (USD or the selected currency)
    ...
    const lineItems = [
      ...cartItems.map((cartItem: any) => {
        const itemPrice = Math.round(cartItem.item.price * 100);
        return {
          price_data: {
            currency: currency,
            product_data: {
      ...
      ...(shippingCost > 0
        ? [
            {
              price_data: {
                currency: currency, // Use selected currency for shipping
                product_data: {
                  name: "Shipping",
